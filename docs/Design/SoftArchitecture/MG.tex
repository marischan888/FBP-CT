...
\item[\refstepcounter{ucnum} \uctheucnum \label{ucI1}:] The core reconstruction
  algorithm is based on FBP.
\item[\refstepcounter{ucnum} \uctheucnum \label{ucI2}:] The mathematical model for
  Radon transform and back-projection.
\item[\refstepcounter{ucnum} \uctheucnum \label{ucI3}:] The type of input data
  being medical CT scans.
\item[\refstepcounter{ucnum} \uctheucnum \label{ucI4}:] The use of Fourier-based
  filtering for noise reduction.
\item[\refstepcounter{ucnum} \uctheucnum \label{ucI5}:] The output data being
  visulized as a reconstructed CT image.
\item[\refstepcounter{ucnum} \uctheucnum \label{ucI6}:] The reliance on matrix
...
\multirow{7}{0.3\textwidth}{Behaviour-Hiding Module} & IO Handler Module\\
& Filters Module\\
& FBP Module\\
\midrule

\multirow{3}{0.3\textwidth}{Software Decision Module} & Services Module\\
...
\item[Services:]Converts the input data into the data structure used by the
  ...
  Handling the reconstruction module and rendering reconstructed image as a 2D array.
...
\item[Secrets:] The filtering algorithms applied to the sinogram, including Ramp,
  Shepp-Logan.
\item[Services:] Applies frequency-domain filtering to enhance image reconstruction
...
\begin{description}
\item[Secrets:] The implementation details of FBP and inverse Radon transform.
...
\begin{description}
\item[Secrets:] The process of generating sinograms using the Radon
  transform, simulating scanner behavior.
\item[Services:] Generates sinograms from test images, injects them into the FBP pipeline, and compares the reconstructed image to the original for evaluation.
\item[Implemented By:] FBP CT Image Reconstruction
\end{description}

\subsubsection{Configuration Module (\mref{mCon})}

\begin{description}
\item[Secrets:] The storage and structure of configurable parameters for filtering,
  image resolution, and reconstruction settings.
\item[Services:] Manages user-defined reconstruction settings, ensuring flexibility
  in system behavior.
\item[Implemented By:] FBP CT Image Reconstruction
\end{description}
