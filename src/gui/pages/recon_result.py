...
class ReconstructionResultPage(tb.Frame):
    def __init__(self, parent, controller):
        ...
        # Title
        self.result_label = tb.Label(self, text="Reconstruction Result", font=("Helvetica", 28, "bold"),
                                     bootstyle="dark")
        self.result_label.grid(row=0, column=0, columnspan=6, pady=(30, 10), sticky="n")

        # === Sinogram Label ===
        self.original_label = tb.Label(self, text="Sinogram", font=("Helvetica", 15), bootstyle="dark")
        self.original_label.grid(row=1, column=0, columnspan=3,pady=(5, 0))

        # === Reconstructed Image Label ===
        self.recon_label = tb.Label(self, text="Reconstructed Image", font=("Helvetica", 15), bootstyle="dark")
        ...
        # === Reconstructed Image ===
        self.recon_canvas = tb.Label(self)
        self.recon_canvas.grid(row=2, column=3, columnspan=3, pady=10)
    ...
    def update_result(self, original, result):
        ...
        self.result = result
        self.original_img = to_tk_image(original, (256,256))
        self.result_img = to_tk_image(self.result, (256,256))


        self.original_canvas.configure(image=self.original_img)
        self.recon_canvas.configure(image=self.result_img)
    ...
    def save_recon(self):
        ...
        if not path or not filename:
            Messagebox.show_error("Error", "Please select a valid path or filename.")
            return
